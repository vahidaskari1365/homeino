...
همچنین یک نکته طراحی داخلی کوتاه (tip) به زبان فارسی ارائه بده.`;
    ...
    // Since Silicon Flow text models don't generate images directly,
    // we need to use an image generation model. Let's try a different approach
    // using their image generation endpoint if available, or use a workaround

    // For now, let's use the Qwen image generation if available
    // If not, we'll return an error suggesting to use a different approach
...
async function generateImageWithSiliconFlow(
  ...
  // Silicon Flow supports image generation through different models
  // Let's try using the image generation endpoint
...
Professional interior design photography, natural lighting, high quality, realistic.`;
  ...
  try {
    ...
    if (!response.ok) {
      // If image generation fails, try fallback approach
...
async function fallbackImageGeneration(
  ...
  // If Silicon Flow image generation is not available,
  // we can use a placeholder or return the original image with modifications
  // For now, let's try using Stable Diffusion XL if available
  ...
  try {
    ...
    // Return a modified version of the original image as last resort
    // In production, you might want to use a different service here
