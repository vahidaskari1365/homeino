$headers = @{ Authorization = "Bearer sbp_6d9d1da0b03a3effc48ec999991d96834bed1363" }
...
begin
  ...
  if v_plan.plan_id is null then
    return jsonb_build_object('allowed', false, 'reason', 'No active subscription', 'code', 'no_subscription');
  end if;
  case p_limit_type
...
foreach ($part in $parts) {
    $i++
    $body = @{ query = $part } | ConvertTo-Json
    try {
        Invoke-RestMethod -Uri "https://api.supabase.com/v1/projects/tljdihejjoepkcgftian/database/query" -Method Post -Body $body -ContentType "application/json" -Headers $headers
        Write-Host "Part $i OK"
    } catch {
        Write-Host "Part $i FAIL: $($_.Exception.Message)"
